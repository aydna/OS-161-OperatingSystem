...
consume_enter(struct resource *resource)
{
	...
	while((resource->num_consumers) >= (resource->ratio * resource->num_producers)) {
		pthread_cond_wait(&(resource->cond), &(resource->mutex)); //we wait for signal that there's space
	}
	
	resource->counter++; // add to thread# of operations
	resource->num_consumers++; // add curr consumer

	// assume no unlock needed cuz its done in the return function?
...
{	
	// pthread_mutex_lock(&(resource->mutex)); not needed? done in function previously

	resource->counter--;
	resource->num_consumers--;
	// signal that there's space on the buffer now
	// do we need this to only be executed when previously full? what if it signals and theres no waiting threads
	pthread_cond_signal(&(resource->cond)); // signal that a spot is now freed
...
produce_enter(struct resource *resource)
{
	...
	pthread_cond_signal(&(resource->cond)); //signal buffer has increased
...
{
    // decrement producer num
	// will need to signal consumer as the ratio is now lowered (might need to sleep one)
